        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- Botón Agregar -->
                    <a href="{{ route('posts.create') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded inline-block mb-4">
                        Agregar
                    </a>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 mt-4">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $post->id }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $post->title }}</td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        <div class="relative">
                                            <span class="block truncate">{{ Str::limit($post->description, 10, '...') }}</span>
                                            @if(Str::length($post->description) > 50)
                                                <a href="#" class="absolute right-0 top-1/2 transform -translate-y-1/2 text-blue-500 hover:text-blue-700 text-xs">Ver más</a>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $post->content }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        @if($post->image)
                                        <img src="{{ asset('images/' . $post->image) }}" alt="{{ $post->title }}" class="w-24 h-16 object-cover">
                                        @else
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                        ...
                                        <form action="{{ route('posts.destroy', $post->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 hover:text-red-700 inline-flex items-center">
